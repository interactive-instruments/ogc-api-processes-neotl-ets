...
77;/conf/job-list/datetime-response;;maybe;;Validate that the datetime query parameter is processed correctly.
78;/conf/job-list/duration-response;;maybe;;Validate that the minDuration and maxDuration query parameter are processed correctly.
...
80;/conf/callback/job-callback;;maybe;Callback implemented Framework, Concept in DSL, but not yet in the Execution Engine;Validate the passing of a subscriber-URL in an execute request.
